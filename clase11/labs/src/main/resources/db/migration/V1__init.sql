...
(
    ...
    request_id      VARCHAR(64)    NOT NULL,
    from_account_id UUID           NOT NULL,
    to_bank_account VARCHAR(32)    NOT NULL, -- CLABE SPEI (18 dígitos) / reservado a 32 por compatibilidad con @Column(length=32)
    to_bank_id      VARCHAR(16)    NOT NULL, -- Clave de institución (3 dígitos) o BIC
    ...
    -- Reglas básicas de integridad según el modelo
    CONSTRAINT payment_currency_chk CHECK (currency ~ '^[A-Z]{3}$'),
    CONSTRAINT payment_state_chk CHECK (state IN ('INITIATED', 'SENT', 'SETTLED', 'FAILED', 'CANCELLED'))
);
